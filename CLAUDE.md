...
**Mission**: Build a complete, production-ready skill package for Vite and publish it under the OpenAEC Foundation on GitHub. This is your standing order for every session in this workspace.
...
**End state**: A published GitHub repo at `https://github.com/OpenAEC-Foundation/Vite-Claude-Skill-Package` with:
...
- Repository topics set (claude, skills, vite, build-tool, frontend, ai, deterministic, openaec)
...
## Project Identity
- Vite skill package for Claude — single technology, frontend build tool (TypeScript + JavaScript)
- Technology: Vite 5.x / 6.x (next-generation frontend build tool with instant HMR, optimized builds, and plugin ecosystem)
...
| docs/masterplan/vite-masterplan.md | Planning | Execution plan with phases, prompts, dependencies |
...
|----------|---------|--------|
| syntax/ | Config syntax, API patterns, plugin API | vite-syntax-{topic} |
| impl/ | Development workflows, integration guides | vite-impl-{topic} |
| errors/ | Error handling, debugging patterns | vite-errors-{topic} |
| core/ | Cross-cutting concerns, architecture | vite-core-{topic} |
| agents/ | Intelligent orchestration | vite-{agent-name} |
...
├── docs/
│   ├── masterplan/              # vite-masterplan.md
│   └── research/                # vooronderzoek-vite.md, topic-research/, fragments/
└── skills/
    └── source/
        ├── vite-syntax/         # Config options, plugin API, HMR API, SSR API
        ├── vite-impl/           # Project setup, library mode, SSR, monorepo
        ├── vite-errors/         # Build errors, dependency issues, HMR failures
        ├── vite-core/           # Architecture, environment API, dev server
        └── vite-agents/         # Validation, config generation agents
```
Single technology package — all skills share the `vite-` prefix.
...
5. **Read docs/masterplan/vite-masterplan.md** → Know the execution plan and current phase details
...
- Vite 5/6 version coverage
- TypeScript and JavaScript coverage for all config/API skills
...
6. TypeScript + JavaScript coverage where applicable
...
### Before ANY research:
1. Read **SOURCES.md** → Know approved sources for Vite
2. Read **REQUIREMENTS.md** → Know what the research must cover
3. Read **DECISIONS.md** → Know constraints (D-003 English-only, D-007 Vite 5/6, D-012 WebFetch, etc.)
...
- Vooronderzoek: `docs/research/vooronderzoek-vite.md`
...
- Naming: `vite-{category}-{topic}`
- TypeScript + JavaScript coverage for config and API skills
...
### Repository: OpenAEC-Foundation/Vite-Claude-Skill-Package

### Pre-Publication Checklist:
1. All skills pass validation (P-003 checklist)
2. INDEX.md is complete and accurate
3. README.md has installation instructions
4. LICENSE file present (MIT)
5. CHANGELOG.md has release entry
6. All references/ files exist and are linked

### Publication Steps:
1. Final validation pass on all skills
2. Update ROADMAP.md to Phase 7 complete
3. Create GitHub release with tag (e.g., v1.0.0)
4. Verify README.md renders correctly on GitHub
5. Submit to agentskills.org registry (if applicable)
